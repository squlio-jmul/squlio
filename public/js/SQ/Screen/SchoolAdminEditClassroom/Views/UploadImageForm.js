define([
	...
	return function UploadImageForm() {
		...
		this.initialize = function($e) {
			...
			_has_uploaded = _$upload_image_form.find('.add-image-container').length ? false : true;
			_setListeners($e);
		};

		this.updateImage = function(url_path) {
			_$upload_image_form.find('.image-preview-container').css('background-image', 'url(' + url_path + ')').fadeIn(300);
			_$upload_image_form.find('label.error').remove();
			_$upload_image_form.find('[name="image_file"]').val('');
			_$upload_image_form.find('.upload-image-form-container').hide();
