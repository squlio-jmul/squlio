		...
		if ($this->cookie->get('id') && $this->cookie->get('type') == 'school_admin') {
			...
			if ($school_id) {
				if ($classroom_obj = $this->classroom_library->get(array('id'=>$classroom_id, 'school'=>$school_id), array(), array(), null, null, array('classroom_teacher'=>true, 'student'=>true, 'classroom_grade'=>true))) {
					...
					$data['term'] = $term_obj;
					$data['jsControllerParam'] = json_encode(array('school_admin_id' => $school_admin['id'], 'classroom_id' => $classroom_id, 'teachers'=>$teacher_obj, 'selected_teacher_ids'=>$selected_teacher_ids, 'primary_teacher_id'=>$primary_teacher_id, 'selected_student_ids'=>$selected_student_ids, 'has_student' => (($student_obj) ? true: false), 'has_term' => (($term_obj) ? true: false), 'has_subject' => (($subject_obj) ? true: false)));
